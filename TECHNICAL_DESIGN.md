...
- 界面在本机浏览器中打开；通过桌面快捷方式启动。
- 本地后台只在 App 使用期间运行。所有 App 标签页关闭后，后台短暂空闲再自动退出。
- 偏好、收藏、文献、已保存译文、标签和笔记必须保存在本机，刷新、关闭浏览器、重启 App 或电脑后均不能丢失。
...
V1 采用“本地网页 + 本地后端 + 本地数据库 + 按需翻译进程”的结构。

```mermaid
flowchart LR
    Shortcut["桌面快捷方式"] --> Backend["本地后端\n127.0.0.1 随机端口"]
    Backend --> Browser["默认浏览器中的 App"]
    Browser <--> Backend
    Backend <--> CoreDB["core.db\n永久数据"]
    Backend <--> CacheDB["cache.db\n最多 500 MB"]
    Backend <--> PdfStore["本地 PDF 资料库\n永久原文件"]
    Browser --> PDFJS["PDF.js 阅读器\n标注层"]
    PDFJS <--> Backend
...
这不是一个一直驻留在系统里的服务。快捷方式启动本地后端并打开浏览器；浏览器页面通过心跳告诉后台“仍在使用”。所有页面都离开后，后台等待 2 分钟，保存任务和数据库状态，然后退出。翻译进程是后台的子进程，后台退出时也必须退出。
...
| PDF 阅读器 | Mozilla PDF.js | 浏览器内完成页面渲染、文本选择、缩放、目录和文内搜索；使用自定义标注层保存 App 内高亮与批注 |
...
1. 用户双击桌面快捷方式。
2. 启动器先检查 App 是否已经运行；已经运行时只打开一个新标签页。
3. 未运行时，在 `127.0.0.1` 上选择一个可用随机端口启动后端。
4. 后端完成数据库检查和过期任务登记后，打开默认浏览器。
5. 浏览器使用一次性启动码换取仅本次运行有效的会话 Cookie，随后从地址栏移除启动码。

不固定使用 3000、8000 等端口，以减少端口冲突。后端只绑定 `127.0.0.1`，局域网里的其他设备不能访问。

### 4.2 判断页面是否仍然打开

- 每个标签页生成独立 `tab_id`，每 15 秒发送一次心跳。
- 页面关闭时用 `sendBeacon` 主动报告，但该信号只作为加速提示，因为浏览器崩溃或电脑休眠时它不一定发送成功。
- 45 秒没有某标签页的心跳，就认为该标签页已经离开。
- 当有效标签页数量为 0 时进入 2 分钟空闲倒计时。
- 倒计时期间重新打开 App，立即取消退出。

### 4.3 安全退出

- 若没有正在写数据库、复制 PDF、OCR 或翻译的任务，倒计时结束后执行数据库检查点并退出。
...
阅读状态采用“每篇文献全局一个状态”，避免同一篇文献在不同知识库出现互相矛盾的阅读进度。标签和笔记既可以是全局的，也可以指定某个知识库，满足项目化整理。
...
1. 新建偏好、收藏、译文和笔记后，刷新、关闭标签页、重启 App、重启电脑，内容都仍存在。
...
5. 所有标签页关闭后约 2 分钟，后端和翻译进程都退出；重新打开标签时取消退出。
6. 强制结束进程或电脑断电模拟后，SQLite 可恢复，已提交的数据不损坏。
7. 备份文件能在一份干净数据目录中恢复并通过完整性检查；由于不含 PDF，恢复后原文件缺失状态与重新关联流程正确。
8. WoS 50 次/天与机构额度场景、OpenAlex 免费额度、Crossref 429 都能正确暂停或退避。
9. 术语表、化学式、单位和 DOI 经翻译后没有被错误改写。
10. 断网时仍可搜索和编辑本地知识库。
11. 同一份 WoS 导出文件重复导入不会产生重复文献，`AB` 摘要能正确进入详情页并保留 `wos_export` 来源。
12. 普通、扫描和混合 PDF 能逐页选择文字提取或 OCR，题目与摘要可预览、修正和翻译。
13. 相同文件重复上传时只保留一份；同 DOI 的不同版本必须提示，不得静默覆盖。
14. PDF 加入多个知识库后只保存一份物理文件，从其中一个知识库移除不会删除原文件。
15. 重启 App 后 PDF 仍可阅读，高亮、划线、批注和笔记能准确回到原页位置。
16. 清理 500 MB 缓存不会删除 PDF 原文件或标注；被清理的 OCR 索引可重新生成。
