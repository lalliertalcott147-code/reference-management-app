...
一个面向个人科研使用的 Windows 本地文献工作台。它在默认浏览器中提供界面，后端、数据库、PDF、译文、笔记和模型全部保存在当前电脑；关闭最后一个 App 标签页后，本地服务会在短暂空闲后退出。
...
然后双击桌面的“催化文献”快捷方式。升级前关闭 App，再运行新版安装脚本。卸载默认保留知识库、PDF、模型和设置；只有明确提供双重删除参数时才会移除永久数据。
