...
export function App() {
  ...
  return <main className="shell"><aside className="sidebar" aria-label="应用侧栏"><div className="brand"><ProfileAvatar avatarUrl={state.settings.avatar_url} onUploaded={(avatarUrl) => setSettings({ ...state.settings, avatar_url: avatarUrl })} /><a className="brand-copy" href="#home"><strong>{state.settings.display_name}</strong><small>文献管理器</small></a></div><nav aria-label="主导航">{navItems.map((item) => <a key={item.page} className={page === item.page ? "active" : ""} href={`#${item.page}`}><span>{item.icon}</span>{item.label}</a>)}</nav><div className="local-badge"><span className="status-dot" /><span>仅本机运行<small>数据保存在此电脑</small></span></div></aside><section className="content">{page === "home" && <HomePage health={state.health} />}{page === "search" && <SearchPage />}{page === "import" && <ImportPage />}{page === "journals" && <JournalsPage />}{page === "library" && <LibraryPage />}{page === "settings" && <SettingsPage settings={state.settings} onChange={setSettings} />}</section>{!state.settings.onboarding_complete && <div className="modal-backdrop"><section className="onboarding" role="dialog" aria-modal="true" aria-labelledby="welcome-title"><span className="brand-mark">RM</span><p className="eyebrow">欢迎使用文献管理器</p><h1 id="welcome-title">这是你的本地文献工作台</h1><ul><li>数据永久保存在当前电脑</li><li>只调用你配置的免费外部服务</li><li>不会模拟学校登录或抓取登录网页</li></ul><button className="primary-button" type="button" onClick={() => { void updateSettings({ onboarding_complete: true }).then(setSettings); }}>知道了，开始使用</button></section></div>}</main>;
}
