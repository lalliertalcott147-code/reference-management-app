...
function NoteEditor({ paper, libraryId }: { paper: LibraryPaper; libraryId?: number }) {
  ...
  return <div className="note-editor"><textarea aria-label={`${paper.title}笔记`} value={body} onChange={(event) => { setBody(event.target.value); setDirty(true); setStatus("等待保存…"); }} placeholder="添加本地笔记…" />{pdfLocation && <a className="reader-link" href={`#reader?paper=${paper.id}&file=${pdfLocation[2]}&page=${pdfLocation[1]}`}>跳回 PDF 第 {pdfLocation[1]} 页</a>}<small role="status">{status}</small></div>;
}
...
export function LibraryPage() {
  ...
  async function loadLibraries(includeDeleted = trashVisible) {
    ...
    setLibraries(result);
    if (selectedLibrary === undefined) setSelectedLibrary(result.find((item) => !item.deleted_at)?.id);
  }
  ...
  return <div className="page library-page"><header className="page-header"><div><p className="eyebrow">永久本地收藏</p><h1>知识库</h1></div><div className="page-actions"><PdfUpload onComplete={() => void loadPapers()} /><button className="secondary-button" type="button" onClick={() => { setTrashVisible((value) => !value); void loadLibraries(!trashVisible); }}>回收站</button></div></header>
    <div className="library-layout"><aside className="library-sidebar"><form onSubmit={(event) => void addLibrary(event)}><input aria-label="新知识库名称" value={newLibrary} onChange={(event) => setNewLibrary(event.target.value)} placeholder="新建知识库" /><button type="submit" aria-label="创建知识库">＋</button></form><button type="button" className={selectedLibrary === undefined ? "active" : ""} onClick={() => setSelectedLibrary(undefined)}>全部文献</button>{activeLibraries.map((library) => <div className="library-link" key={library.id}><button type="button" className={selectedLibrary === library.id ? "active" : ""} onClick={() => setSelectedLibrary(library.id)}><span>{library.name}</span><small>{library.paper_count}</small></button><button type="button" aria-label={`删除 ${library.name}`} onClick={() => void deleteLibrary(library.id).then(() => loadLibraries())}>×</button></div>)}</aside>
      <section className="library-main"><form className="library-search" onSubmit={(event) => { event.preventDefault(); void loadPapers(query); }}><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索题名、作者、摘要、译文、标签和笔记" /><button className="primary-button" type="submit">本地搜索</button></form>
